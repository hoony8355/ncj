    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Object.prototype.hasOwnProperty.call(responseData, 'errors')) {
          throw new Error(responseData["errors"].map((error: { message: string }) => error.message).join(", "));
        } else {
          throw new Error('양식 제출 중 오류가 발생했습니다.');
        }
      }
    } catch (error) {
      setFormState({ status: 'error', message: (error as Error).message || '알 수 없는 오류가 발생했습니다. 잠시 후 다시 시도해주세요.' });
    }
  };
